...
var _ = Describe("Operators", func() {
	...
	DescribeTable("ToSQL",
		...
		Entry("tag contains multiple values", Contains{"genre": []string{"Dance", "Rock"}}, "(exists (select 1 from json_tree(tags, '$.genre') where key='value' and value LIKE ?) AND exists (select 1 from json_tree(tags, '$.genre') where key='value' and value LIKE ?))", "%Dance%", "%Rock%"),
		...
		Entry("role contains multiple values", Contains{"artist": []string{"u2", "brian eno"}}, "(exists (select 1 from json_tree(participants, '$.artist') where key='name' and value LIKE ?) AND exists (select 1 from json_tree(participants, '$.artist') where key='name' and value LIKE ?))", "%u2%", "%brian eno%"),
