...
func syncDailyMixPlaylistsBestEffort(ctx context.Context, ds model.DataStore, seed string, sections []homeRecommendationsSection) {
	...
	// Build track IDs for each Daily Mix based on the albums selected for the Home buckets.
	// Even if a mix is empty, we still create the playlist so it is visible.
	albumMixes := map[int]model.Albums{}
	...
	for i := 1; i <= 3; i++ {
		...
		// Aim for a longer playlist than the album bucket: cap to avoid huge payloads.
		maxTracks := len(albums) * 10
		if maxTracks < 25 {
			maxTracks = 25
		}
		if maxTracks > 150 {
